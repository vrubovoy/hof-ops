...
// Privileged opt-in is enforced by the thin test/apply-acceptance.mjs
// entrypoint, which dynamic-imports THIS module only after
// HOF_ALLOW_PRIVILEGED_ACCEPTANCE=1 is confirmed - so nothing here
// (imports included) runs on a plain `pnpm test:apply-ssh`. This module
// is deliberately not matched by `test/*.test.mjs` (the `.impl.mjs`
// suffix), so `pnpm test` never loads it either.
